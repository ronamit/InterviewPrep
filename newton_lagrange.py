...
def run_augmented_lagrangian_method(n_dim, m_constraints, func, f_grad, f_hess, constraints_vec, constraints_jacobian, inner_solver, n_iter=100,
    ...
    for i in range(n_iter):
        ...
        phi = lambda _x: func(_x) + (eta / 2) * np.sum(constraints_vec(_x) ** 2) + np.sum(lamb * constraints_vec(_x))
        ...
        phi_grad = lambda _x: f_grad(_x) \
                             + eta * constraints_vec(_x) @ constraints_jacobian(_x) \
...
x = run_augmented_lagrangian_method(n_dim, m_constraints, quadratic_func, quadratic_func_grad, quadratic_func_hess,
                                    quadratic_func_constr, quadratic_func_constr_jacobian, run_newton_method, n_iter=20)
